...
observations. 
} {\bf \color{darkgreen} MJV: Added this sentence to emphasize why ABC is advantageous when systematic are present. @CHH: Can you take a look at this?} {\bf \color{dred} The systematic uncertainties may affect the likelihood in an unknown way, but their impact can be simulated and marginalized out in the forward generative model of the ABC-PMC method.}
...
discrepancy may explain the less biased constraints on $\alpha$ from the ABC-PMC analysis 
compared to the Gaussian-likelihood analysis in \ref{fig:cont_nbargmf}.
}{\bf \color{darkgreen} @CHH : added $\alpha$ to the previous sentence since the other constraints our consistent!}
...
{\bf \color{dred} In standard likelihood analyses, these selection effects, if not accounted for, can lead to biases in the inferred parameters of interest. Methods that rely on a likelihood function may not correctly predict how the functional form of the likelihood is impacted by the presence of possible selection effects and systematic uncertainties.  
