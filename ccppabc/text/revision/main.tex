...
{\bf \color{dred}
In Figures~\ref{fig:hist_nbarxi} and \ref{fig:hist_nbargmf}, for each of the parameters, 
the marginalized posteriors from both inference methods are comparable. Similarly, 
the lower panels of the figures reveal that the constraints derived for each parameter
from the two methods are also consistent, with minor discrepancies. Furthermore, we 
find that both methods produce constraints consistent with the ``true'' HOD parameters.
}
{\bf \color{darkgreen}
CHH: A few sentences discussing the width of the distributions for each of the panels
have been commented out. Not sure if it's meaningful to include such slight discrepancies. 
...
%%%
{\bf \color{dred} 
Overall, the constraints from ABC-PMC are consistent with those from 
...
{\bf \color{dred}
It is possible to consider a scenario in which the results from the standard analysis are inconsistent with the results from the ABC. Let us consider a dataset---from a galaxy survey---consisting of objects with different data qualities, signal-to-noise ratios, and systematic effects. This could be a catalog of galaxies which is incomplete beyond a certain luminosity or redshift. Another example could be a catalog with a singal-to-noise ratio cut. 
}

{\bf \color{dred}
In standard likelihood analyses, these selection effects, if not accounted for, can lead to biases in the inferred parameters of interest. Hierarchical models can modify the likelihood functions through by taking into account the selection effects. Alternatively, one could alleviate this issue by employing ABC equipped with a generative forward model of the data that incorporates signal-to-noise ratio cuts, incompleteness, etc.
}{\bf \color{darkgreen}
MJV: @CHH Can you read the two paragraphs above and see if they address referee's concern about the extreme data situation that could lead to discrepancy ?
}
%%%
...
%%%
{\bf \color{darkgreen}
MJV: @CHH I think this paragraph is misleading since both ABC and Gaussian likelihood covariances suffer from inaccuracy of the simulations. I say we remove this paragraph entirely. It does not add any clarify anything. It just causes confusion!
}
{\bf \color{darkgreen} 
CHH: @MJV I'm not a big fan of this entire paragraph. The accuracy of the covariance
...
\begin{figure*}
%\begin{center}
\includegraphics[width=0.95\textwidth]{figs/paper_ABC_poolevolution_nbargmf.pdf}
