...
{\bf \color{darkgreen} 
CHH: @MJV I incorporated the sentence into the paragraph. Can you check it out
and confirm it's fine?
} 
...
assumption is incorrect for 2PCF.
}
{\bf \color{dred}
...
clearly misrepresents the true likelihood function. This incorrect likelihood, 
may explain why the constraints on $\alpha$ are less biased for the ABC-PMC analysis 
than the Gaussian-likelihood analysis in \ref{fig:cont_nbargmf}.
}
...
Gaussian pseudo-likelihood analysis, more realistic scenarios face mmany factors
that can generate inconsistencies. Consider a typical galaxy catalog from 
observations for LSS. These catalogs consist of objects with different data 
...
{\bf \color{dred} 
These selection effects, coupled with the systematic effects mentioned above, 
make correctly predicting the functional form of the likelihood difficult. 
In the standard Gaussian pseudo-likelihood analysis, and other analysis that
depend on writing down a likelihood function, these effects can siginficantly 
bias the inferred parameters of interest. In these situations, employing
